...
const ExcelJS = require('exceljs');const puppeteer = require('puppeteer');
const fs = require('fs');
const path = require('path');
const nodemailer = require('nodemailer');
const { JSDOM } = require('jsdom');
...
async function waitForDownloadAndRename(downloadPath, newFileName) {
    ...
    // ตรวจสอบขนาดไฟล์เบื้องต้น และแสดงเป็น KB
    const stats = fs.statSync(oldPath);
    const fileSizeInKB = (stats.size / 1024).toFixed(2);
    console.log(`   ✅ Found File: ${downloadedFile} (Size: ${fileSizeInKB} KB)`);
...
async function convertHtmlToExcel(sourcePath, destPath) {
    try {
        console.log(`   Converting HTML-XLS to Beautiful XLSX...`);
        ...
        console.log(`   ✅ Conversion success: ${path.basename(destPath)}`);
    } catch (e) {
...
(async () => {
    ...
    console.log('🚀 Starting DTC Automation (KB Size Log)...');
